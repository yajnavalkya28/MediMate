...
from groq_utils import initialize_groq, generate_response, create_initial_prompt, ask_clarifying_question, format_history
...
def main():
    ...
    age = st.number_input("Age", min_value=1, max_value=120, value=30, step=1, format="%d")
    gender = st.selectbox("Gender", options=["Male", "Female", "Other"])
    weight = st.number_input("Weight (kg)", min_value=1, max_value=300, value=70, step=1, format="%d")
    ...
    if prompt := st.chat_input("Describe your health problem..."):
        ...
        with st.chat_message("assistant"):
            ...
            if len(st.session_state.messages) == 1:
                initial_prompt = create_initial_prompt(translated_prompt, age=age, gender=gender, weight=weight, height=height, medical_history=medical_history, location=location, bmi=bmi)
                response = generate_response(model, initial_prompt, history=[])
            ...
            message_placeholder.markdown(translated_response, unsafe_allow_html=True)
